...
func getEnableBackendKeepalive(service *v1.Service) (bool, error) {
	...
	enableProxyProtocol, err := strconv.ParseBool(enableBackendKeepaliveStr)
	if err != nil {
		return false, fmt.Errorf("failed to backend keepalive flag %q from annotation %q: %s", enableBackendKeepaliveStr, annDOEnableBackendKeepalive, err)
	}
